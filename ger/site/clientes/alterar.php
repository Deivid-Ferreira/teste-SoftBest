	...
	if($_COOKIE['loginAprovado'.$cookie] != ""){
		...
		if($controleUsuario == "tem usuario"){
			...
			if($validaAcesso == "ok"){
				...
				$sqlNomePagamento = "SELECT codCliente, nomeCliente, statusCliente FROM clientes WHERE codCliente = '".$url[6]."' LIMIT 0,1";
...
?>
				<div id="localizacao-topo">
					<div id="conteudo-localizacao-topo">
						...
						<p class="flexa"></p>
						<p class="nome-lista"><?php echo $dadosNomePagamento['nomeCliente'] ;?></p>
						<br class="clear" />
					</div>
					<table class="tabela-interno" >
<?php
				if($dadosNomePagamento['statusCliente'] == "T"){
					$status = "status-ativo";
					$statusIcone = "ativado";
...
?>	
						<tr class="tr-interno">
							<td class="botoes-interno"><a href='<?php echo $configUrl; ?>site/clientes/ativacao/<?php echo $dadosNomePagamento['codCliente'] ?>/' title='Deseja <?php echo $statusPergunta ?> o parceiro <?php echo $dadosNomePagamento['nomeCliente'] ?>?' ><img src="<?php echo $configUrl; ?>f/i/default/corpo-default/<?php echo $status ?>-branco.gif" alt="icone"></a></td>
							<td class="botoes-interno"><a href='javascript: confirmaExclusao(<?php echo $dadosNomePagamento['codCliente'] ?>, "<?php echo htmlspecialchars($dadosNomePagamento['nomeCliente']) ?>");' title='Deseja excluir o parceiro <?php echo $dadosNomePagamento['nomeCliente'] ?>?' ><img src='<?php echo $configUrl; ?>f/i/default/corpo-default/excluir-branco.gif' alt="icone"></a></td>
						</tr>
						<script>
...
<?php
				if(isset($_POST['alterar'])){
					...
					$sql = "UPDATE clientes SET nomeCliente = '".preparaNome($_POST['nome'])."', linkCliente = '".$_POST['link']."', urlCliente = '".$urlCliente."' WHERE codCliente = '".$url[6]."'";
					...
					if($result == 1){
						...
						$erroData = "<p class='erro'>Problemas ao alterar parceiro!</p>";
					}
				}else{
					$sql = "SELECT * FROM clientes WHERE codCliente = ".$url[6];
					$result = $conn->query($sql);
					$dadosCliente = $result->fetch_assoc();
					$_SESSION['nome'] = $dadosCliente['nomeCliente'];
					$_SESSION['link'] = $dadosCliente['linkCliente'];
				}
...
?>				
						...
						<form action="<?php echo $configUrlGer; ?>site/clientes/alterar/<?php echo $url[6] ;?>/" method="post">
