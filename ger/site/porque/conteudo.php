...
?>
				<div id="filtro">
					<div id="localizacao-filtro">
						...
						<p class="flexa"></p>
						<p class="nome-lista">Por que contratar</p>
						<br class="clear" />
					</div>
					<div class="demoTarget">
						<div id="formulario-filtro">
							...
							<form name="filtro" action="<?php echo $configUrl;?>site/porque/" method="post" />
				...
				if(isset($_POST['cadastrar'])){
					...
					if($result == 1){
						if(isset($_POST['cadastrar'])){
							...
							echo "<meta HTTP-EQUIV='Refresh' CONTENT='0;URL=".$configUrlGer."site/porque/'>";
						}else{
...
?>				
						...
						<br/>
						<form action="<?php echo $configUrlGer; ?>site/porque/" method="post">
...
?>
								<tr class="tr">
									...
									<td class="oitenta"><a href='<?php echo $configUrlGer; ?>site/porque/alterar/<?php echo $dadosPorque['codPorque'] ?>/' title='Veja os detalhes da por que contratar <?php echo $dadosPorque['nomePorque'] ?>'><?php echo $dadosPorque['nomePorque'];?></a></td> 
									<td class="botoes" style="width:5%;"><a style="padding:0px;" href='<?php echo $configUrl; ?>site/porque/imagens/<?php echo $dadosPorque['codPorque'] ?>/' title='Deseja gerenciar imagens do banner <?php echo $dadosPorque['nomePorque'] ?>?' ><img style="<?php echo $dadosImagem['codPorqueImagem'] == "" ? 'display:none;' : 'padding-top:10px;';?>" src="<?php echo $configUrlGer.'f/porque/'.$dadosImagem['codPorque'].'-'.$dadosImagem['codPorqueImagem'].'-O.'.$dadosImagem['extPorqueImagem'];?>" height="45"/><img style="<?php echo $dadosImagem['codPorqueImagem'] != "" ? 'display:none;' : '';?>" src="<?php echo $configUrl; ?>f/i/default/corpo-default/gerenciar-imagens.gif" alt="icone"></a></td>
									<td class="botoes"><a href='<?php echo $configUrl; ?>site/porque/ativacao/<?php echo $dadosPorque['codPorque'] ?>/' title='Deseja <?php echo $statusPergunta ?> a por que contratar <?php echo $dadosPorque['nomePorque'] ?>?' ><img src="<?php echo $configUrl; ?>f/i/default/corpo-default/<?php echo $status ?>.gif" alt="icone"></a></td>
									<td class="botoes"><a href='<?php echo $configUrl; ?>site/porque/alterar/<?php echo $dadosPorque['codPorque'] ?>/' title='Deseja alterar a por que contratar <?php echo $dadosPorque['nomePorque'] ?>?' ><img src="<?php echo $configUrl;?>f/i/default/corpo-default/icones-alterar.gif" alt="icone" /></a></td>
							...
							new Sortable(dragArea, {
								...
								onEnd: function(e) {
									...
									for (i = 0; i < myLength; i++) {
										cont++;
										$tgb.post("<?php echo $configUrlGer;?>site/porque/ordena.php", {codPorque: allinputs[i].value, codOrdenacaoPorque: cont});
									}
								}
							});				
						</script>	
