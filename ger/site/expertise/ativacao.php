		...
		if($controleUsuario == "tem usuario"){
			...
			if($validaAcesso == "ok"){
				...
				if($url[5] != ""){
					...
					$sqlCons = "SELECT nomeExpertise, statusExpertise FROM expertise WHERE codExpertise = ".$url[6]." LIMIT 0,1";
					$resultCons = $conn->query($sqlCons);
					$dadosCons = $resultCons->fetch_assoc();
					
					echo "<div id='filtro'>";
				
					if($dadosCons['statusExpertise'] == "T"){
					...
					$sql =  "UPDATE expertise SET statusExpertise = '".$alteraStatus."' WHERE codExpertise = ".$url[6];
					$result = $conn->query($sql);
				
					if($result == 1){
						$_SESSION['nome'] = $dadosCons['nomeExpertise'];
