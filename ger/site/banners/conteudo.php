...
<?php
				if(isset($_POST['cadastrar'])){
					...
					$sql = "INSERT INTO banners VALUES(0, ".$novoOrdem.", '".preparaNome($_POST['nome'])."', '".$_POST['link']."', '".$_POST['novaAba']."', 'T', '".$urlBanner."')";
...
<?php
					if($url[5] == 1 || $url[5] == ""){
						$pagina = 1;
						$sqlBanner = "SELECT * FROM banners ORDER BY statusBanner ASC, codOrdenacaoBanner ASC, codBanner DESC LIMIT 0,30";
					}else{
						$pagina = $url[5];
						$paginaFinal = $pagina * 30;
						$paginaInicial = $paginaFinal - 30;
						$sqlBanner = "SELECT * FROM banners ORDER BY statusBanner ASC, codOrdenacaoBanner ASC, codBanner DESC LIMIT ".$paginaInicial.",30";
					}		
...
?>
								<tr class="tr">
									<td class="dez" style="text-align:center;"><input type="number" class="campo" style="width:30px; text-align:center; border:1px solid #0000FF; outline:none;" value="<?php echo $dadosBanner['codOrdenacaoBanner'];?>" onClick="alteraCor(<?php echo $dadosBanner['codBanner'];?>);" onBlur="alteraOrdem(<?php echo $dadosBanner['codBanner'];?>, this.value);" id="codOrdena<?php echo $dadosBanner['codBanner'];?>"/></td>
...
?>
								...
								<script type="text/javascript">
									...
									function alteraOrdem(cod, ordem){
										...
										$po.post("<?php echo $configUrlGer;?>site/banners/ordem.php", {codBanner: cod, codOrdenacaoBanner: ordem}, function(data){		
