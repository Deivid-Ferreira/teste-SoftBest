				...
				<div id="filtro">
					...
					<div class="demoTarget">
						<div id="formulario-filtro">
							...
							<form name="filtro" action="<?php echo $configUrl;?>site/equipe/" method="post" />
...
<?php
				if(isset($_POST['cadastrar']) || isset($_POST['cadastrarFotos'])){
					...
					$sql = "INSERT INTO equipe VALUES(0, ".$novoOrdem.", '".preparaNome($_POST['nome'])."',  '".$_POST['celular']."',  '".preparaNome($_POST['subNome'])."', '".str_replace("'", "&#39;", $descricao)."', 'T', '".$urlEquipe."')";
					...
					if($result == 1){
						if(isset($_POST['cadastrar'])){
							...
							echo "<meta HTTP-EQUIV='Refresh' CONTENT='0;URL=".$configUrlGer."site/equipe/imagens/".$dadosEquipe['codEquipe']."/'>";
						}
					}else{
...
?>				
						...
						<br/>
						<form action="<?php echo $configUrlGer; ?>site/equipe/" method="post">
							<p class="bloco-campo-float"><label>Nome: <span class="obrigatorio"> * </span></label>
							<input class="campo" type="text" name="nome" style="width:320px;" required value="<?php echo $_SESSION['nome']; ?>" /></p>

							<p class="bloco-campo-float"><label>Celular / WhatsApp: <span class="obrigatorio"> </span></label>
							<input id="celular" class="campo" type="text" name="celular" style="width:235px;" value="<?php echo $_SESSION['celular']; ?>" onKeyDown="Mascara(this,novoTelefone);" onKeyPress="Mascara(this,novoTelefone);" onKeyUp="Mascara(this,novoTelefone);"/></p>
...
?>
								<tr class="tr">
									...
									<td class="setenta"><a href='<?php echo $configUrlGer; ?>site/equipe/alterar/<?php echo $dadosEquipe['codEquipe'] ?>/' title='Veja os detalhes do integrante da equipe <?php echo $dadosEquipe['nomeEquipe'] ?>'><?php echo $dadosEquipe['nomeEquipe'];?></a></td>
									<td class="dez" style="text-align:center;">--</td>
									<td class="dez" style="text-align:center;">--</td>
									<td class="botoes"><a href='<?php echo $configUrl; ?>site/equipe/alterar/<?php echo $dadosEquipe['codEquipe'] ?>/' title='Deseja alterar o integrante da equipe <?php echo $dadosEquipe['nomeEquipe'] ?>?' ><img src="<?php echo $configUrl;?>f/i/default/corpo-default/icones-alterar.gif" alt="icone" /></a></td>
									<td class="dez" style="text-align:center;">--</td>
								</tr>
<?php						
						}else{
?>
								<tr class="tr">
									...
									<td class="oitenta"><a href='<?php echo $configUrlGer; ?>site/equipe/alterar/<?php echo $dadosEquipe['codEquipe'] ?>/' title='Veja os detalhes do integrante da equipe <?php echo $dadosEquipe['nomeEquipe'] ?>'><?php echo $dadosEquipe['nomeEquipe'];?></a></td>
									<td class="botoes" style="width:5%;"><a style="padding:0px;" href='<?php echo $configUrl; ?>site/equipe/imagens/<?php echo $dadosEquipe['codEquipe'] ?>/' title='Deseja gerenciar imagens do integrante da equipe <?php echo $dadosEquipe['nomeEquipe'] ?>?' ><img style="<?php echo $dadosImagem['codEquipeImagem'] == "" ? 'display:none;' : 'padding-top:5px;';?>" src="<?php echo $configUrlGer.'f/equipe/'.$dadosImagem['codEquipe'].'-'.$dadosImagem['codEquipeImagem'].'-W.webp';?>" height="50"/><img style="<?php echo $dadosImagem['codEquipeImagem'] != "" ? 'display:none;' : '';?>" src="<?php echo $configUrl; ?>f/i/default/corpo-default/gerenciar-imagens.gif" alt="icone"></a></td>
									<td class="botoes"><a href='<?php echo $configUrl; ?>site/equipe/ativacao/<?php echo $dadosEquipe['codEquipe'] ?>/' title='Deseja <?php echo $statusPergunta ?> o integrante da equipe <?php echo $dadosEquipe['nomeEquipe'] ?>?' ><img src="<?php echo $configUrl; ?>f/i/default/corpo-default/<?php echo $status ?>.gif" alt="icone"></a></td>
									<td class="botoes"><a href='<?php echo $configUrl; ?>site/equipe/alterar/<?php echo $dadosEquipe['codEquipe'] ?>/' title='Deseja alterar o integrante da equipe <?php echo $dadosEquipe['nomeEquipe'] ?>?' ><img src="<?php echo $configUrl;?>f/i/default/corpo-default/icones-alterar.gif" alt="icone" /></a></td>
...
?>
								<script>
									function confirmaExclusao(cod, nome){
										if(confirm("Deseja excluir o integrante da equipe "+nome+"?")){
											window.location='<?php echo $configUrlGer; ?>site/equipe/excluir/'+cod+'/';
										}
									}
								</script>
								<script type="text/javascript">
									...
									function alteraOrdem(cod, ordem){
										...
										$po.post("<?php echo $configUrlGer;?>site/equipe/ordem.php", {codEquipe: cod, codOrdenacaoEquipe: ordem}, function(data){		
